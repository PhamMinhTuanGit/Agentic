...
def retriever(query, top_k=25):
    results = search_with_indexes(
        query, index_dir="indexes", top_k=top_k, alpha=0.4, verbose=True
    )
    return results
...
def construct_prompt(query, results):
    ...
    prompt = template.create_search_augmented_prompt(query, results, max_results=10)
    return prompt
...
def retriever_with_rerank(query, top_k=25, rerank_top_k=10):
    ...
    reranked_results = reranker.rerank_batch(
        query, results, batch_size=8, top_k=rerank_top_k
    )
